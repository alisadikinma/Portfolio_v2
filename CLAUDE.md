...
├── LinkedInGenerationService.php # SSH-invokes `/linkedin-gen` CLI, parses stdout JSON, advances FSM, dispatches GenerateLinkedInCarouselImages on carousel format. When `linkedin.use_carousel_gen_engine=true` AND format=carousel, also dispatches `/carousel-gen` engine via `applyCarouselGenAdapter()` and replaces slides via `CarouselGenOutputAdapter`
...
### LinkedIn Carousel Engine Decoupling (April 28, 2026 — Phase A complete, flag OFF)
...
- **Feature flag** — [`config/linkedin.php`](backend/config/linkedin.php) gains `use_carousel_gen_engine` key (env: `LINKEDIN_USE_CAROUSEL_GEN_ENGINE`, default false). Flag OFF = legacy `/linkedin-carousel` inline path (current production). Flag ON = router branches to `/carousel-gen` engine. Tests: 5 feature tests in [`LinkedInGenerationServiceCarouselGenRouterTest`](backend/tests/Feature/LinkedInGenerationServiceCarouselGenRouterTest.php) cover flag-off pass-through, text-format pass-through, carousel-format slide replacement, `status=failed` exception, dispatch-null exception. Plus 25 carousel-gen related tests passing with 318 assertions.
...
**Phase B — Cutover (next session, operator-driven):**
- B1: Smoke test 1 manual draft with flag temporarily ON for one worker (~10 min, costs ~$0.50-1.40 in tokens)
- B2: Flip `LINKEDIN_USE_CAROUSEL_GEN_ENGINE=true` globally on VPS
- B3: 7-day monitoring window (Telegram alerts, FSM corruption audit, daily failed count check)
- B4: Update CLAUDE.md + PROJECT_STATUS.md with cutover marker
- B5: Rollback (~5 min flag-flip back to false) if P0/P1 incident detected

**Phase C — Cleanup (post-B success):**
- C1: Move `linkedin-post-writer/skills/linkedin-carousel/` → `deprecated/`
- C2: Drop `refs-linkedin-carousel.md` from `compile-refs.ts`
- C3: Refactor `/linkedin-gen` orchestrator to text-only path (carousel format → `route_to_carousel_gen` envelope status)
- C4: Remove `LINKEDIN_GEN_REFS_CAROUSEL` env + config + service references
- C5: Bump `linkedin-post-writer` v0.5.0 (BREAKING: `/linkedin-carousel` removed)
- C6: Bump `ai-image-carousel-prompt-gen` v3.0.0 (universal carousel engine GA)
...
# Carousel Gen Engine — Phase A feature-flagged OFF (April 28, 2026)
# When flag flips to true (Phase B cutover), carousel format dispatches /carousel-gen
# instead of legacy /linkedin-carousel inline path. SSH bridge mirrors LINKEDIN_GEN_*.
LINKEDIN_USE_CAROUSEL_GEN_ENGINE=false                      # Phase A=false, Phase B=true
