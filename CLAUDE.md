...
- ✅ Compatibility: legacy envelopes with `status=complete` + `format=carousel` + inline slides still work (test `test_legacy_complete_carousel_envelope_replaces_slides`) — adapter replaces inline slides with /carousel-gen output
- 🟡 VPS deploy pending operator: `git pull` plugin to v0.5.0 commit, `npm install + npm run compile-refs` (now produces 3 files), remove `/home/claudesn/refs-linkedin-carousel.md` symlink, drop `LINKEDIN_USE_CAROUSEL_GEN_ENGINE` + `LINKEDIN_GEN_REFS_CAROUSEL` from Laravel `.env`, `php artisan config:cache && queue:restart`
