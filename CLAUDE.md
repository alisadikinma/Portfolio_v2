...
|---|---|---|---|
| `homepage` | `hero` | [Home.vue](frontend/src/views/Home.vue) | `HeroOperator` (person-forward montage video + ALI SADIKIN MA wordmark + manifesto + 3 CTAs + stat triad; **geminigen-generated hero still `public/videos/hero-poster.jpg` wired as `posterSrc`** = immediate render + reduced-motion/slow-conn fallback; interim `hero-bg.mp4` loop until operator renders final per [operator-render-brief.md](docs/plans/hero-video/operator-render-brief.md). Replaced `CinematicHero` June 8.) |
...
| `homepage` | `international-stages` | Home.vue | `InternationalStages` (6 curated stage cards: Bengaluru/Hangzhou-UNCTAD/Silicon Valley/NextDev/Fenox/IDBYTE — curated narrative + **real event photos pulled per `awardId` via [`useStageGalleries`](frontend/src/composables/useStageGalleries.js) from `/api/awards/{id}/galleries`; card click opens `BaseGalleryModal`**; text-only fallback if a gallery is empty) |
