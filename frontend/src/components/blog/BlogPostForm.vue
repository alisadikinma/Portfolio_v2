...
<template>
  <form class="blog-post-form space-y-6" @submit.prevent="handleSubmit('draft')">
    ...
    <div class="flex items-center gap-2 border-b border-gray-200 dark:border-gray-700 pb-3">
      <span class="text-sm font-medium text-gray-500 dark:text-gray-400 mr-2">Language:</span>
      <button
        v-for="lang in [{code:'en',label:'English (EN)',flag:'🇺🇸'},{code:'id',label:'Indonesian (ID)',flag:'🇮🇩'}]"
        :key="lang.code"
        type="button"
      ...
      >
        ...
        {{ lang.label }}
        <span
          v-if="translationData[lang.code]?.title"
          class="w-2 h-2 rounded-full"
          :class="activeTranslationTab === lang.code ? 'bg-white/60' : 'bg-green-500'"
        ></span>
      </button>
      <span v-if="activeTranslationTab === 'id'" class="ml-auto text-xs text-amber-600 dark:text-amber-400">
        Tip: Keep tech terms in English (deploy, API, framework)
      </span>
    </div>
