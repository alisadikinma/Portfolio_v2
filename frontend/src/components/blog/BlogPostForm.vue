...
const formData = ref({
  ...
  focus_keyword: '',
  canonical_url: '',
  // For edit mode: store translations with IDs
...
const handleSubmit = async (status = 'draft') => {
  ...
  const submitData = {
    ...
    meta_keywords: formData.value.focus_keyword || null,  // Map focus_keyword to meta_keywords
    og_image: null,  // Will be handled separately if needed
    schema_markup: null,  // Will be auto-generated by backend
    faq_schema: null,  // Will be auto-generated by backend
    seo_score: 0,  // Will be calculated by backend
    index_follow: true,  // Default to true
    // Per-language SEO fields (in post_translations)
    translations: formData.value.translationsData
      ? formData.value.translationsData.map(trans => ({
          id: trans.id,  // Include ID for update
      ...
      : [
          // Default EN translation for new posts
          {
            language: 'en',
            title: formData.value.title,
            slug: formData.value.slug,
...
const loadPostData = (post) => {
  ...
  console.log('[BlogPostForm] Loading post data:', post)
  ...
  formData.value = {
    ...
    status: post.published ? 'published' : 'draft',  // Fix: use published boolean
    published_at: post.published_at || null,
    // Fix: SEO fields are nested under post.seo
    meta_title: post.seo?.meta_title || '',
    meta_description: post.seo?.meta_description || '',
    focus_keyword: '',  // Not used in backend, keep empty
    canonical_url: post.seo?.canonical_url || '',
    // Store raw translations for edit mode
    translationsData: post.translations || null
  }
