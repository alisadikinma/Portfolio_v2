    ...
    <div v-else-if="error" class="py-12">
      <BaseCard>
        <div class="text-center">
          ...
          <BaseButton @click="router.push('/admin/testimonials')">
    ...
    <div v-else-if="testimonial">
      ...
      <BaseCard>
        <TestimonialForm
          ...
          submit-label="Update Testimonial"
...
async function fetchTestimonial() {
  ...
  try {
    ...
    if (fetchedTestimonial && fetchedTestimonial.client_name) {
      testimonial.value = fetchedTestimonial
    } else {
      throw new Error('Invalid testimonial data received')
    }
  } catch (err) {
    console.error('Failed to fetch testimonial:', err)
    error.value = err.response?.data?.message || 'Failed to load testimonial. Please try again.'
  } finally {
    loading.value = false
  }
}
...
async function handleSubmit(testimonialData) {
  ...
  try {
    ...
    uiStore.showSuccess(
      `Testimonial from "${updatedTestimonial.client_name}" has been updated successfully.`,
      'Testimonial Updated'
    )
    ...
    uiStore.showError(
      err.response?.data?.message || 'Failed to update testimonial. Please try again.',
      'Update Failed',
      0 // Persistent
    )
  } finally {
