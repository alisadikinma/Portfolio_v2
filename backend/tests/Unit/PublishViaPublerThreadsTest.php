...
/**
 * TDD — PublishViaPubler real implementation for Threads (P4, 2026-05-13).
 */
class PublishViaPublerThreadsTest extends TestCase
{
    use RefreshDatabase;

    private function makeSibling(): ThreadsPost
    {
        $category = Category::create(['name' => 'AI', 'slug' => 'ai-th-' . uniqid()]);
        $post = Post::create([
            'category_id' => $category->id,
            'slug' => 'test-post-th-' . uniqid(),
            'title' => 'Test Post Threads',
            'content' => 'content',
        ]);

        $slides = [
            ['slide_number' => 1, 'image_url' => 'https://cdn.test/th-s1.png', 'is_cover' => true, 'is_cta' => false],
            ['slide_number' => 2, 'image_url' => 'https://cdn.test/th-s2.png', 'is_cover' => false, 'is_cta' => true],
        ];
    ...
    private function setPublerApiKey(): void
    {
        Setting::create([
            'group' => 'publer',
            'key' => 'publer_api_key',
            'value' => Crypt::encryptString('test-api-key-threads'),
        ]);
    }
    ...
    {
        $this->setSetting('publer_threads_account_id', 'th_acc_test');
        ...
        $job = new PublishViaPubler('threads', $sibling->id);
    ...
    public function test_skips_when_publer_post_id_already_set(): void
    {
        ...
        $sibling = $this->makeSibling();
        $sibling->update(['publer_post_id' => 'threads_already_published', 'status' => 'published']);

        $job = new PublishViaPubler('threads', $sibling->id);
        $job->handle(app(PublerClient::class), app(PublerPayloadBuilder::class));

        $fresh = $sibling->fresh();
        $this->assertSame('threads_already_published', $fresh->publer_post_id);
        $this->assertSame('published', $fresh->status);
    }
}
